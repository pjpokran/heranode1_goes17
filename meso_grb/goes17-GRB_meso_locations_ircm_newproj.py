...
f2k1 = f2.variables['planck_fk1'][0]
f2k2 = f2.variables['planck_fk2'][0]
b2c1 = f2.variables['planck_bc1'][0]
b2c2 = f2.variables['planck_bc2'][0]
#print("fk1 = ",fk1)
#print("fk2 = ",fk2)
#print("bc1 = ",bc1)
#print("bc2 = ",bc2)
...
#  T = [ fk2 / (alog((fk1 / L1) + 1)) - bc1 ] / bc2
data_var = (fk2 / ( np.log((fk1 / data_var) + 1 )) - bc1) / bc2
data_var2 = (f2k2 / ( np.log((f2k1 / data_var2) + 1 )) - b2c1) / b2c2
...
#asmall = scipy.misc.imresize(a,0.1,interp='nearest')
#a2small = scipy.misc.imresize(a2,0.1,interp='nearest')
...
#xasmall = np.arange(0,len(xa),10)
xasmall = xa[0::10]
print(xa)
print("shape",xasmall.shape)
print(xasmall)
yasmall = ya[0::10]


#xa2small = scipy.signal.resample(xa2,50)
#ya2small = scipy.signal.resample(ya2,50)
...
image2_rows=f2.dimensions["y"].size
image2_columns=f2.dimensions["x"].size
...
proj = ccrs.Geostationary(central_longitude=f3.variables['nominal_satellite_subpoint_lon'][0],
                          satellite_height=f3.variables['nominal_satellite_height'][0] * 1000,
                          globe=globe,sweep_axis='x')
print("central longitude, nominal satellite subpoint lon ",f3.variables['nominal_satellite_subpoint_lon'][0],
                          f3.variables['nominal_satellite_height'][0] * 1000)

print("proj",proj)

# Define proj2 to be the lambert conformal projection that NOAAPORT CONUS was in at first
#proj2 = ccrs.PlateCarree(central_longitude=-106.0,
#                             globe=globe)
#proj2 = ccrs.Miller(central_longitude=-106.0)
#proj2 = ccrs.Mollweide(central_longitude=-40)
#proj2 = ccrs.Mercator(central_longitude=0.,min_latitude=-70.0, max_latitude=70.0, latitude_true_scale=0.)
proj2 = ccrs.Geostationary(central_longitude=-75.,
                          satellite_height=f3.variables['nominal_satellite_height'][0] * 1000,
                          globe=globe,sweep_axis='x')

print("geospatial_lat_lon_extent", f.variables['geospatial_lat_lon_extent'])
print("geospatial_lat_lon_extent].geospatial_lat_center", f.variables['geospatial_lat_lon_extent'].geospatial_lat_center)
print("geospatial_lat_lon_extent].geospatial_lon_center", f.variables['geospatial_lat_lon_extent'].geospatial_lon_center)
...
# Wisconsin fullres crop
fig90 = plt.figure(figsize=(2.0,2.0))
...
ax9 = fig9.add_subplot(1, 1, 1, projection=proj2)
#ax9 = fig9.add_subplot(1, 1, 1, projection=proj)
ax9.outline_patch.set_edgecolor('red')
##ax9.set_extent((-213.5,-65.0,-25.0,65.0))
#ax9.set_extent([-213.5,-70.0,0.0,85.0])
##ax9.set_extent([-140,-100.0,-90.0,90.0],crs=ccrs.PlateCarree())
#ax9.set_global()
...
#ax11.gridlines()
ax90 = fig90.add_subplot(1, 1, 1, projection=proj2)
ax90.outline_patch.set_edgecolor('none')
#ax90.set_extent((-179.9,-105.0,0.0,85.0))
#ax90.set_extent((-179.0,-105.0,-40.0,85.0))
ax90.set_global()
...
#im = ax9.imshow(a3[:], extent=(xa3[1],xa3[-1],ya3[-1],ya3[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330.)

#from cartopy.img_transform import warp_array
#
#result_array, extent = warp_array(a, source_proj=proj, target_proj=proj2, target_res=(200,100))

#im = ax9.imshow(a[:], extent=(xa[1],xa[-1],ya[-1],ya[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330., transform=proj)
print("start imshow")
im = ax9.imshow(asmall[:], extent=(xasmall[1],xasmall[-1],yasmall[-1],yasmall[1]), transform = proj, origin='upper', cmap=my_cmap, vmin=162., vmax=330.)
#im = ax9.imshow(a[:], extent=(xa[1],xa[-1],ya[-1],ya[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330., transform=proj)
#im = ax9.imshow(asmall[:], origin='upper', cmap=my_cmap, vmin=162., vmax=330., transform=proj)
#im = ax9.imshow(asmall[:], extent=(xasmall[1],xasmall[-1],yasmall[-1],yasmall[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330.)
print("done  imshow")
#im = ax9.imshow(asmall[:], extent=(xasmall[1],xasmall[-1],yasmall[-1],yasmall[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330., transform=proj)
#im = ax9.imshow(a2[:], extent=(xa2[1],xa2[-1],ya2[-1],ya2[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330., transform=proj)
#im = ax90.imshow(a[:], extent=(xa[1],xa[-1],ya[-1],ya[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330.)
#im = ax90.imshow(a2[:], extent=(xa2[1],xa2[-1],ya2[-1],ya2[1]), origin='upper', cmap=my_cmap, vmin=162., vmax=330.)
...
#stn6.set_path_effects(outline_effect)
# HERE PLOT LAT/LON of CENTER
#ax9.plot(, 38.5458, 'bo', markersize=3, transform=ccrs.Geodetic())
print("[geospatial_lat_lon_extent].geospatial_lat_center", f.variables['geospatial_lat_lon_extent'].geospatial_lat_center)
print("[geospatial_lat_lon_extent].geospatial_lon_center", f.variables['geospatial_lat_lon_extent'].geospatial_lon_center)
print("[geospatial_lat_lon_extent].geospatial_westbound_longitude", f.variables['geospatial_lat_lon_extent'].geospatial_westbound_longitude)
print("[geospatial_lat_lon_extent].geospatial_eastbound_longitude", f.variables['geospatial_lat_lon_extent'].geospatial_eastbound_longitude)
print("[geospatial_lat_lon_extent].geospatial_northbound_latitude", f.variables['geospatial_lat_lon_extent'].geospatial_northbound_latitude)
print("[geospatial_lat_lon_extent].geospatial_southbound_latitude", f.variables['geospatial_lat_lon_extent'].geospatial_southbound_latitude)

# N/W
#plotlon=round(f.variables['geospatial_lat_lon_extent'].geospatial_westbound_longitude + 1.0)
#plotlat=round(f.variables['geospatial_lat_lon_extent'].geospatial_northbound_latitude - 1.0)
#printlonlat = str(int(plotlon))+","+str(int(plotlat))
#stn9 = ax9.text(plotlon, plotlat, printlonlat, transform=ccrs.Geodetic(), color='darkorange')
#stn9.set_path_effects(outline_effect)
## S/W
##plotlon=round(f.variables['geospatial_lat_lon_extent'].geospatial_westbound_longitude + 1.0)
##plotlat=round(f.variables['geospatial_lat_lon_extent'].geospatial_southbound_latitude + 1.0)
##printlonlat = str(int(plotlon))+","+str(int(plotlat))
##stn9 = ax9.text(plotlon, plotlat, printlonlat, transform=ccrs.Geodetic(), color='darkorange')
##stn9.set_path_effects(outline_effect)
### N/E
##plotlon=round(f.variables['geospatial_lat_lon_extent'].geospatial_eastbound_longitude - 1.0)
##plotlat=round(f.variables['geospatial_lat_lon_extent'].geospatial_northbound_latitude - 1.0)
##printlonlat = str(int(plotlon))+","+str(int(plotlat))
##stn9 = ax9.text(plotlon, plotlat, printlonlat, transform=ccrs.Geodetic(), color='darkorange')
##stn9.set_path_effects(outline_effect)
...
## Center
# Plot 1 on meso 1
plotlon=round(f.variables['geospatial_lat_lon_extent'].geospatial_lon_center)
plotlat=round(f.variables['geospatial_lat_lon_extent'].geospatial_lat_center)
#printlonlat = str(int(plotlon))+","+str(int(plotlat))
stn9 = ax9.text(plotlon, plotlat, "Meso1", fontsize=14, horizontalalignment='center', transform=ccrs.Geodetic(), color='darkorange')
stn9.set_path_effects(outline_effect)
stn90 = ax90.text(plotlon, plotlat, "1", horizontalalignment='center', transform=ccrs.Geodetic(), color='darkorange')
stn90.set_path_effects(outline_effect)
# Plot 2 on meso 2
plotlon=round(f2.variables['geospatial_lat_lon_extent'].geospatial_lon_center)
plotlat=round(f2.variables['geospatial_lat_lon_extent'].geospatial_lat_center)
#printlonlat = str(int(plotlon))+","+str(int(plotlat))
stn9 = ax9.text(plotlon, plotlat, "Meso2", fontsize=14, horizontalalignment='center', transform=ccrs.Geodetic(), color='darkorange')
stn9.set_path_effects(outline_effect)
stn90 = ax90.text(plotlon, plotlat, "2", horizontalalignment='center', transform=ccrs.Geodetic(), color='darkorange')
stn90.set_path_effects(outline_effect)
##
##
...
ctime_file_string = iyear + imonth + iday + itimehr + itimemn + itimesc + "_globe"
list_string = sunique + '.jpg'

time_string = 'GOES-17 Clean IR LW window (ABI ch 13)\n %s\nPRELIMINARY NON-OPERATIONAL DATA'%ctime_string
print(time_string)
...
##
text9 = ax9.text(0.01, 0.93, time_string,
...
filename9="/whirlwind/goes17/grb/meso_ircm/"+ctime_file_string+".jpg"
filename9l="/whirlwind/goes17/grb/meso_ircm/latest_meso_globe.jpg"
#filename9t="/tornado/home2/goes16_images/grb/meso_ircm/"+ctime_file_string+".jpg"
filename90="/whirlwind/goes17/grb/meso_ircm/latest_meso_globe_s.jpg"
...
fig9.savefig(filename9l, bbox_inches='tight',pad_inches=0, facecolor='black')
...
shutil.copy(filename9, "/whirlwind/goes17/grb/meso_ircm/latest_meso_1.jpg")
#shutil.copy(filename9, filename9t)

import glob
file_list = glob.glob('/whirlwind/goes17/grb/meso_ircm/*' + list_string)
file_list.sort()
#print("file list is ",file_list)

thefile = open('/whirlwind/goes17/grb/meso_ircm/meso_1_30_temp.list', 'w')
...
for item in thelist:
    ...
    thefile.write(tail + '\n')
thefile.close
os.rename('/whirlwind/goes17/grb/meso_ircm/meso_1_30_temp.list','/whirlwind/goes17/grb/meso_ircm/meso_1_30.list')

thefile = open('/whirlwind/goes17/grb/meso_ircm/meso_1_60_temp.list', 'w')
...
for item in thelist:
    ...
    thefile.write(tail + '\n')
thefile.close
os.rename('/whirlwind/goes17/grb/meso_ircm/meso_1_60_temp.list','/whirlwind/goes17/grb/meso_ircm/meso_1_60.list')
